...
class UDPHandler {
  ...
  static StreamController<dynamic> _eventsStreamControllerJsonDecode =
      StreamController.broadcast();
  static Stream<dynamic> get eventsStreamJsonDecode =>
      _eventsStreamControllerJsonDecode.stream;

  static dynamic? EventsStream;
        ...
        _udpSocket!.listen((RawSocketEvent event) {
          if (event == RawSocketEvent.read) {
            ...
            if (datagram != null) {
              ...
              _logToHive(logEntry);
              //print(XORDecode);
              EventsStream = XORDecode;
              _eventsStreamController.add(XORDecode);
              _eventsStreamControllerJsonDecode.add(jsonDecode(XORDecode));
            }
          }
        });
      });
  ...
  static void _logToHive(LogEntry logEntry) async {
    ...
    logBox.add(logEntry).whenComplete(() {
      print(
          'Log entry added successfully: DEN ${jsonDecode(logEntry.data)["DEN"]} & ID-${jsonDecode(logEntry.data)["ID"]} at ${logEntry.time}');
    });
  }
